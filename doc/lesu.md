...
The firmware controls response of the pump depending on input on channels 1, 2 and 5. The main goal is to make the operation smooth and fluid comparing to a simple channel mix in the transmitter. The conventional approach does not take into account the fact that there is some travel distance before a valve begins to open at `VALVE_MIN`. In this model, there is no practical need to account for half-opened state because the valves open quickly. A better strategy is to keep the pump off until a valve is open and then divide the input by `VALVE_DIV` to equalize response. As a bonus, there is no need to create a channel mix on the transmitter for the pump.
