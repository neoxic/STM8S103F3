...
void update(void) {
	...
	if (b != bm) { // Change mode atomically
		TIM2_CR1 = 0x00; // Disable counter
		TIM2_CCMR1 = 0x40; // OC1M=100 (force low)
		TIM2_CCMR3 = 0x40; // OC3M=100 (force low)
		if (b & 1) TIM2_CCMR1 = 0x30; // OC1M=011 (toggle)
		if (b & 2) TIM2_CCMR3 = 0x30; // OC3M=011 (toggle)
		if (b & 4) { // Strobe
