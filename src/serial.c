...
void initserial(void) {
	...
	TIM4_SR = 0x00; // Avoid immediate UE after UG
...
// 3) Upon receiving a sensor request (and if a response is required), RX is first turned off
...
void UART_TXE(void) __interrupt(UART_TXIRQ) {
	if ((UART_SR & 0x40) && (UART_CR2 & 0x40)) { // TC=1 && TCIEN=1
